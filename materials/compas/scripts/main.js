...
function assessModel(d, threshold){
    ...
    for(var i = 0; i < predictions.length; i++){
        ...
        if(called_high_risk[i]) {
            proportion_called_high_risk++;
        }
        if(called_high_risk[i] == d[i][d[i].length - 1]) { // was the prediction accurate?
            accuracy++;
        } 
        if (called_high_risk[i] && called_high_risk[i] != d[i][d[i].length - 1]){
            false_positives++;
        } 
        else if (!called_high_risk[i] && called_high_risk[i] == d[i][d[i].length - 1]) {
            true_negatives++;
        }
    }
    ...
    console.log('The false positive rate is %f.', parseFloat(false_positive_rate.toFixed(2)));
...
function assessModelByRace(d, threshold){
    ...
    console.log("------------AFRICAN AMERICAN ASSESMENT--------------")
...
d3.csv("../compas-scores-two-years-violent.csv", function(d){
    ...
    // 1. Let's try fitting other columns
    // Try changing "sex" to other columns in our csv file and run your program again.
    // Notice any differences in the proportion called high risk? 
    // Is sex a good metric to train on?
